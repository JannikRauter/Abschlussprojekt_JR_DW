...
class NMCBattery(BatteryPack):
    """ Modell eines NMC-Akkupacks mit 10 seriellen Zellen. """

    SERIES_CELLS: int = 10
    CELL_RESISTANCE_OHM: float = 0.007
    ...
    def __init__(
        ...
        """ Initialisiert das NMC-Akkupack. """
    ...
    def ocv(self) -> float:
        ...
        assert 32.0 <= open_circuit_voltage <= 42.0, (
            "Die NMC-Leerlaufspannung liegt außerhalb "
            "des gültigen Bereichs von 32 bis 42 V."
        )

        logger.debug(
            "NMC-OCV berechnet: SOC=%.2f %%, OCV=%.2f V",
            self.soc * 100.0,
